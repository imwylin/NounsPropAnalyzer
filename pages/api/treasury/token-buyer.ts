...
interface TokenBuyerResponse {
  transactions: Transaction[];
  balances: TokenBalance[];
}

export default async function handler(req: NextApiRequest, res: NextApiResponse<TokenBuyerResponse | { error: string }>) {
  ...
  try {
    ...
    console.log('Fetching token buyer data...');
    ...
    console.log('Processing token buyer data...');
    ...
    const processedTransactions = transactions.map(tx => 
      processTransaction(tx, 'tokenBuyer')
    );

    console.log('Token buyer data processed successfully');
    ...
    console.error('Token Buyer API Error:', error);
    const errorMessage = error instanceof Error ? error.message : 'Unknown error occurred';
    res.status(500).json({ error: `Failed to fetch token buyer data: ${errorMessage}` });
  }
} 